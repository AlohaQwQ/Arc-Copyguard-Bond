...
* `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` 缺失时，`npm run build` 不能失败；
...
6. 缺少 `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` 时 build 不能失败；
...
合约里的 leaderId 是 `bytes32("hl_leader_03")`，即 ASCII 字符串右填充零：
...
7. 缺少 `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` 时 build 不能失败；
...
5. `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` 缺失不导致构建失败；
6. 不修改 `contracts/`；
7. 不修改 `agents/`；
