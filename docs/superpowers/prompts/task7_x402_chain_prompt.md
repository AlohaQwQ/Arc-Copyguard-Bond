...
| 不修改 | `agents/risk-worker/llm.py`、`scoring.py`、`schemas.py`、`mock_data.py`、`config.py` |
| 不修改 | `contracts/*`、`apps/*` |
...
chain.py 初始化时检查：
```python
if not settings.RISK_ORACLE_ADAPTER_ADDRESS:
    # chain submission 不可用，但 API 仍可工作（返回 report 不含 tx_hash）
```
...
def submit_risk_update(bond_id: int, risk_score_bps: int, report_hash: str) -> str:
    ...
    tx = adapter.functions.submitRiskUpdate(
        bond_id,
        risk_score_bps,
        bytes.fromhex(report_hash[2:])  # "0xabcd..." → bytes
    ).build_transaction({
...
- submit_risk_update 应抛出明确的 ValueError 或返回 None
- 调用方（main.py）负责处理此情况
...
9. 验证失败 → 返回 (False, 具体错误信息)
10. 验证成功 → 返回 (True, "verified")
...
- 链上提交失败不应导致 API 请求失败（tx_hash 为 None 即可）
- x402 验证必须同时检查：tx status、contract address、event user、leaderId
- chainId 验证：5042002
...
| 8 | verify_payment 检查：tx status, contract address, event user, leaderId | 读文件 |
...
| 13 | schemas.py RiskReport 新增 txHash 字段 | 读 schemas.py |
