...
如果新增 `home-*` 组件，最终输出必须说明为什么需要拆分；禁止借 Task 9B 重构既有业务组件。
...
- CTA 按钮（使用 Next.js `Link`，**不使用** `Button asChild`）：
  ...
  - View Events → `/events`
- WalletConnect 组件可保留：如果当前首页已引入，可以作为辅助入口保留；但首页内容不得依赖钱包连接。若当前首页未引入，不要为了 Task 9B 新增钱包逻辑
...
- 使用 Tailwind；可复用已有轻量 UI 组件，但不要引入新依赖，不要使用 `Button asChild`
- 使用 Next.js `Link` 做页面跳转
- WalletConnect 组件可以保留在 Hero section（它是 client component，但 page 本身保持 server component）；但不得让首页内容依赖钱包连接
...
- Next.js 15 前端已完成，P0 核心 demo 业务链路已实现
...
**Section 2: Demo Flow**
- 4 步流程：
  1. Review Leaders — Browse 5 mock leaders with demo risk scores on /leaders
  2. Create Bond — Connect wallet, select a leader, create a protection bond with Arc native USDC
  3. Unlock Report — Pay 1 USDC on-chain via x402-style flow to unlock the full AI risk report
  4. Track Events — Watch real on-chain events (BondCreated, ReportPurchased, RiskUpdated) on /events
- 每步用数字 + 标题 + 简短描述
...
以上禁止范围检查命令必须无输出。允许 `git diff --stat` 总览里只出现 `apps/web/app/page.tsx`，以及最多两个可选新增的 `apps/web/components/home-*.tsx`。
...
1. 修改了哪些文件
2. 是否只修改 `apps/web/app/page.tsx`，或是否新增了 `home-*` 静态组件
3. 首页新增/增强了哪些 section
...
| 30 | `apps/web/app/page.tsx` 是主要修改文件 | `git diff --stat` |
| 31 | 如新增 `home-*` 组件，输出说明原因 | Codex 输出 |
| 32 | npm run build 无 TypeScript 错误 | build 输出确认 |
