# Task 6: LLM Rationale + Deterministic Fallback — 完整执行上下文（修订版）
...
- 根据 `LLM_PROVIDER` 选择 SDK：
  - `"claude"` → 使用 `anthropic` SDK（需用户自行 `pip install anthropic`）
  - `"openai"` → 使用 `openai` SDK（需用户自行 `pip install openai`）
  - `"gemini"` → 使用 `google.genai` SDK（需用户自行 `pip install google-genai`）
- **必须 lazy import SDK**：不要在 `llm.py` 顶部 import 这些 SDK；只在对应 provider 分支内部 import。SDK 未安装时捕获 `ImportError` / `ModuleNotFoundError` 并 fallback。
...
- 未安装 LLM SDK 时，服务仍必须能启动，并走 deterministic fallback
- `llm.py` 顶部不得 import anthropic/openai/google.genai
...
1. **LLM SDK 必须 lazy import。**
   - 不要在 `llm.py` 顶部 import `anthropic`、`openai`、`google.genai`。
   - 只能在对应 provider 分支内部 import。
   - 如果 SDK 未安装，必须捕获 `ImportError` / `ModuleNotFoundError` 并 fallback 到 `scoring.generate_reasons()`。
   - 未配置 `LLM_PROVIDER` / `LLM_API_KEY` 时，FastAPI 必须仍然可以正常启动。

2. **未配置 LLM 时必须完全保持 Task 5 行为。**
   `LLM_PROVIDER` 未设置或为空时，同一 `leaderId + bondId` 多次请求必须满足：
   - `riskScoreBps` 相同；
   - `action` 相同；
   - `bondAction` 相同；
   - `recommendedAllocationBps` 相同；
   - `confidenceBps` 相同；
   - `reasons` 来自 deterministic `generate_reasons()`；
   - `reportHash` 一致。

3. **配置 LLM 后，LLM 也只能影响 reasons。**
   即使 LLM 返回了 `riskScoreBps`、`action`、`bondAction`、`recommendedAllocationBps`、`confidenceBps` 等字段，也必须全部忽略，只读取并校验 `reasons`。

4. **reportHash 稳定性规则。**
   - `LLM_PROVIDER` 未设置时，`reportHash` 必须对同一 `leaderId + bondId` 稳定一致。
   - `LLM_PROVIDER` 设置时，因为 reasons 文案可能变化，`reportHash` 可以随 reasons 变化；但 `riskScoreBps/action/bondAction/recommendedAllocationBps/confidenceBps` 必须稳定。
   - Task 6 的验收默认以 `LLM_PROVIDER` 未设置的 fallback 模式为准。

5. **日志不能泄露敏感信息。**
   warning log 不得输出 `LLM_API_KEY`、`Authorization` header、完整请求 header、私钥、完整 `.env` 内容。只允许输出 provider、错误类型、`fallback to deterministic reasons` 等非敏感信息。

6. **`GET /api/reports/{leader_id}` 只做 402 预留。**
   该 endpoint 不调用 LLM、不生成完整 report、不提交链上交易，只检查 leader 是否存在，然后返回 402 payment required payload。

7. **不要修改 `requirements.txt`。**
   如果用户未来想启用真实 LLM provider，由用户手动安装对应 SDK。Task 6 默认 fallback 模式必须不依赖这些 SDK。
...
测试（Windows PowerShell 推荐使用 `Invoke-RestMethod`，避免 `curl` alias 问题）：
```powershell
# 1. 不设置 LLM_PROVIDER（deterministic fallback）
$r1 = Invoke-RestMethod `
  -Method POST `
  -Uri "http://localhost:8000/api/oracle/run-risk-check" `
  -Headers @{ "Content-Type" = "application/json" } `
  -Body '{"leaderId":"hl_leader_03","bondId":1}'

# 应返回完整 RiskReport，reasons 来自 deterministic 路径
$r1.riskScoreBps
$r1.action
$r1.bondAction
$r1.reportHash
$r1.reportHash.Length  # 应为 66，即 0x + 64 hex

# 2. 同一 leader + bondId 连续两次请求，riskScoreBps 和 reportHash 必须一致
...
$r1.riskScoreBps -eq $r2.riskScoreBps  # True
$r1.reportHash -eq $r2.reportHash      # True

# 3. 新的 /api/reports endpoint，应返回 402
try {
  Invoke-RestMethod "http://localhost:8000/api/reports/hl_leader_03"
} catch {
  $_.Exception.Response.StatusCode.value__
}

# 4. 不存在的 leader，应返回 404
try {
  Invoke-RestMethod "http://localhost:8000/api/reports/hl_leader_nonexist"
} catch {
  $_.Exception.Response.StatusCode.value__
}
```

如果使用真正的 curl，请使用 `curl.exe` 而不是 PowerShell alias `curl`：
```powershell
curl.exe -X POST "http://localhost:8000/api/oracle/run-risk-check" `
  -H "Content-Type: application/json" `
  --data-raw '{"leaderId":"hl_leader_03","bondId":1}'
```
...
| 18 | LLM SDK lazy import | 读 `llm.py`，确认顶层没有 `import anthropic/openai/google.genai` |
| 19 | 未安装 LLM SDK 时服务可启动 | 不设置 `LLM_PROVIDER`，运行 `from main import app` 成功 |
| 20 | fallback 模式 reportHash deterministic | 同一 `leaderId + bondId` 请求两次，`reportHash` 一致 |
| 21 | LLM 返回额外字段会被忽略 | 读代码，确认只读取并校验 `reasons` |
| 22 | warning log 不泄露 key/header | 读代码，确认不打印 `LLM_API_KEY` / Authorization header |
| 23 | `/api/reports/{leader_id}` 不调用 LLM | 读 main.py，确认只检查 leader 并返回 402 |
...
1. 创建/修改了哪些文件；
2. `llm.py` 是否使用 lazy import；
3. 未设置 `LLM_PROVIDER` 时，导入检查是否通过；
4. `POST /api/oracle/run-risk-check` fallback 模式是否返回完整 `RiskReport`；
5. 同一 `leaderId + bondId` 请求两次，`riskScoreBps` 和 `reportHash` 是否一致；
6. `GET /api/reports/hl_leader_03` 是否返回 402；
7. 不存在 leader 是否返回 404；
8. 是否未修改 `requirements.txt`、`scoring.py`、`schemas.py`、`mock_data.py`、`config.py`；
9. 是否未修改 `contracts/` 和 `apps/`；
10. 如果有失败项，只给出最小修复建议。
